...
def data_generation() -> tuple[list[str], list[str]]:
    ...
    questions: list[str] = []
    # Initialize a list storing the expected summation answers.
    expected: list[str] = []
    ...
    for encoding_layer, encoding_neurons in enumerate(ENCODE_NEURONS):
        if encoding_layer == 0:
            ...
            model.add(keras.layers.LSTM(
                encoding_neurons, return_sequences=True))
        else:
            model.add(keras.layers.LSTM(encoding_neurons))
...
def train_model(ctable: CharacterTable, model: keras.Sequential, Xtrain: NDArray[int32], Ytrain: NDArray[int32], Xtest: NDArray[int32], Ytest: NDArray[int32], EPOCHS: int, BATCH_SIZE: int, DEMONSTRATION_SIZE: int):
...
def main():
    ...
    # -----------------------------------------------------------------------------
    Xtrain, Ytrain, Xtest, Ytest = data_vectorization(
        ctable, questions, expected)
    ...
    training_loss, training_accuracy, testing_loss, testing_accuracy = train_model(
        ctable, model, Xtrain, Ytrain, Xtest, Ytest, EPOCHS, BATCH_SIZE, DEMONSTRATION_SIZE)
